...
export const Route = createFileRoute("/_authenticated/customer")({
  head: () => ({
    meta: [{ title: "My rides — ONE WAY CAB" }, { name: "robots", content: "noindex" }],
  }),
...
function CustomerHub() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("bookings")
      .select(
        "id,booking_ref,pickup_city,drop_city,pickup_date,pickup_time,vehicle_name,estimated_fare,status,trip_type",
      )
      ...
      <main className="mx-auto max-w-7xl px-4 pt-28 pb-16 sm:px-6">
        ...
        <header className="mb-6 flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex gap-1 rounded-full border border-white/10 bg-white/[0.03] p-1 text-xs">
            <button
              onClick={() => setTab("book")}
              className={`rounded-full px-4 py-1.5 ${tab === "book" ? "bg-[color:var(--gold)]/15 text-[color:var(--gold)]" : "text-muted-foreground"}`}
            >
              Book new
            </button>
            <button
              onClick={() => setTab("trips")}
              className={`rounded-full px-4 py-1.5 ${tab === "trips" ? "bg-[color:var(--gold)]/15 text-[color:var(--gold)]" : "text-muted-foreground"}`}
            >
        ...
        {hasLead === false && (
          <Link
            to="/lead"
            className="mb-6 flex items-center justify-between gap-4 rounded-2xl border border-[color:var(--gold)]/30 bg-[color:var(--gold)]/10 p-4 hover:bg-[color:var(--gold)]/15"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Tell us about your trip (20s) and we'll issue a personal coupon.
                </div>
              </div>
            </div>
        ...
        {hasLead && couponCode && (
          <Link
            to="/coupon"
            className="mb-6 flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4 hover:bg-white/[0.06]"
          >
            <div className="flex items-center gap-3">
              <Ticket className="h-6 w-6 text-[color:var(--gold)]" />
              <div>
                <div className="text-sm font-semibold">
                  Your coupon{" "}
                  <span className="font-mono text-[color:var(--gold)]">{couponCode}</span>
                </div>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1.5 flex items-center gap-2 text-sm font-semibold">
                        <MapPin className="h-4 w-4 text-[color:var(--gold)]" /> {t.pickup_city} →{" "}
                        {t.drop_city}
                      </div>
                      <div className="mt-1 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                        ...
                        {t.estimated_fare != null && (
                          <span className="inline-flex items-center gap-0.5">
                            ...
                            {Number(t.estimated_fare).toLocaleString("en-IN")}
                          </span>
                        )}
                      </div>
                    </div>
                    <Link
                      to="/book"
                      search={{ pickup: t.pickup_city, drop: t.drop_city }}
